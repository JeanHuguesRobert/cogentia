...
- **Semantic embeddings**
  ...
  * `scripts/smart-embed-worker.js` and `scripts/import-embeddings.js` — inspect or process embedding continuations through the AI-router embedding endpoint, optionally in resumable `--max-batches` slices, then import the result through the CLI.

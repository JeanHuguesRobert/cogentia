...
- Before Git writes, inspect `git status --short --branch`, then run `git fetch origin --prune` and compare the current branch with its upstream. Fetching is the mandatory read-only freshness check. Use `git pull --ff-only` only when the checkout is clean and simply behind; never use a blind pull to resolve local work. When WIP or a concurrent change is present, reconcile directly while preserving both contributions (for example `rebase --autostash` when applicable).

...
cognitive packets; continuation; multi-agent systems; agent orchestration; human-AI cooperation; copy/paste; pass by value; pass by reference; payload schema; resumability; traceability; Cogentia; second method; agent-resumable CLI.
...
This problem appears in several ordinary cases:

- a user wants to move work from one AI conversation to another;
- an agent must hand over a partially completed task to another agent;
- a CLI tool reaches a point requiring semantic judgment;
- a todo item must preserve enough context to be executable later;
- a GitHub issue must capture not only a request, but the state of reasoning that produced it;
- a human reviewer must resume a task without rereading a long thread;
- a future digital twin must continue a line of work without silently rewriting its assumptions.
...
Without these fields, an agent or human receiving a task often receives an instruction without its epistemic boundary.
...
A cognitive packet may refer to resources exposed by such a protocol, but it does not replace the protocol.
...
A cognitive packet can be produced, routed, consumed, archived, or validated by such frameworks, but it is not itself a runtime.
...
A cognitive packet differs from ordinary workflow state.
...
A cognitive packet is primarily prospective.
...
```json
{
  ...
  "next_action": "Refine the minimal schema and test it on copy/paste, GitHub issue, and CLI continuation use cases.",
  "traces": [
    {
      "type": "path",
  ...
  "resumption_risks": [
    ...
    "Expanding the scope into governance or general AI safety."
  ]
}
```
...
# 11. Relation to Agent-Resumable CLI
...
# 12. Relation to Cogentia and the Second Method
...
## 13.1 Conversation-to-conversation handoff
...
The receiving agent can resume work without reconstructing the entire prior conversation.

## 13.2 Conversation-to-todo handoff
...
## 13.3 Conversation-to-GitHub issue
...
When I ask for a "continuation by copy", produce a standalone packet that can be pasted into another conversation, todo list, issue, document, tool, or repository.
...
Always distinguish established state, decisions, assumptions, constraints, next action, traces, and resumption risks.
...
## 16.1 Summary masquerading as continuation
...
## 16.4 Decision hidden as assumption
...
## 18.4 Over-trust in generated packets
...
Conditions:
  ...
  D. Receiver gets cognitive packet by reference plus access to shared context.
...
The essential rule is equally simple:

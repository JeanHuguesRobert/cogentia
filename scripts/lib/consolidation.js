...
export async function runWeeklyConsolidation(options = {}) {
  ...
  // Fetch recent git commit history for cross-check
  const gitLogs = {};
  const targetRepos = [
  ...
  for (const repo of targetRepos) {
    if (fs.existsSync(repo.path)) {
      try {
        const logText = execFileSync("git", ["log", "--since=7 days ago", "--oneline", "-n", "10"], {
          cwd: repo.path,
          encoding: "utf8",
        });
        gitLogs[repo.name] = logText.trim().split("\n").filter(Boolean);
      } catch {
        gitLogs[repo.name] = [];
      }
    }
...
function generateWeeklyDigestMarkdown({ sprintTag, timestamp, gitStatus, indexStatus, traceSync, projectionResult, gitLogs = {} }) {
  const cogentiaCommits = (gitLogs.cogentia || []).map(c => `- \`${c}\``).join("\n");
  const insemeCommits = (gitLogs.inseme || []).map(c => `- \`${c}\``).join("\n");
  const baronsCommits = (gitLogs["barons-Mariani"] || []).map(c => `- \`${c}\``).join("\n");
...
### Repository: \`cogentia\`
${cogentiaCommits || "- *No recent commits*"}

### Repository: \`inseme\`
${insemeCommits || "- *No recent commits*"}

### Repository: \`barons-Mariani\`
${baronsCommits || "- *No recent commits*"}

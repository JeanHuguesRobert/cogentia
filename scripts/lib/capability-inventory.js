...
export const LEGACY_SYMMETRY_CAPABILITIES = [
  ...
  },
  {
    ...
    description: "List a bounded directory on a selected host node (provider-backed; not a DC tool name)",
    projections: {
      human_cli: { available: false, target: "not yet; Reality Test #184" },
      human_web_ux: { available: false, target: "later: browser.fractavolta.com / Agent John" },
      machine_mcp: { available: true, target: "cogentia_host_fs_list (private-read)" },
      machine_api_v1: { available: false, target: "not a public daemon route" },
      cop_packet: { available: true, target: "cogentia.host_capability_trace/v1" },
    },
  },
  {
    name: "host.fs.read",
    category: "host",
    maturity: "experimental",
    description: "Read a bounded file on a selected host node (provider-backed; not a DC tool name)",
    projections: {
      human_cli: { available: false, target: "not yet; Reality Test #184" },
      human_web_ux: { available: false, target: "later: browser.fractavolta.com / Agent John" },
      machine_mcp: { available: true, target: "cogentia_host_fs_read (private-read)" },
      machine_api_v1: { available: false, target: "not a public daemon route" },
      cop_packet: { available: true, target: "cogentia.host_capability_trace/v1" },
    },
  },
  {
    ...
    category: "host",
    maturity: "experimental",
    description: "Search a bounded host directory (provider-backed; not a DC tool name)",
    projections: {
      human_cli: { available: false, target: "not yet; Reality Test #184" },
      human_web_ux: { available: false, target: "later: browser.fractavolta.com / Agent John" },
      machine_mcp: { available: true, target: "cogentia_host_fs_search (private-read)" },
      machine_api_v1: { available: false, target: "not a public daemon route" },
      cop_packet: { available: true, target: "cogentia.host_capability_trace/v1" },
    },
  },

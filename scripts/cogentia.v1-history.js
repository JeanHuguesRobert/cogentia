...
const COMMAND_MANIFEST = [
  ...
  {
    name: "install-hooks", description: "Retired. Removes legacy Cogentia pre-commit hooks if still present (no new install).",
    parameters: { type: "object", properties: {} },
...
function cmdInstallHooks() {
  // Retired: the status-based pre-commit gate was non-enforcing tech debt.
  // This command only removes legacy hooks; it no longer installs any.
  const { configPath, config } = loadConfig();
  if ( !configPath ) die( "No registry found. Run: cogentia add <repo> first." );
  const removed = [];
  ...
  for (const entry of config.repos) {
    ...
    let touched = false;
    for (const name of ["pre-commit", "pre-commit.cmd", "pre-commit.old"]) {
      const target = path.join(hooksDir, name);
      if (!fs.existsSync(target)) continue;
      try {
        const text = fs.readFileSync(target, "utf8");
        const isOurs =
          text.includes("cogentia.js") ||
          text.includes("install-hooks") ||
          text.includes("[Cogentia]") ||
          text.includes("node_modules/pre-commit/hook");
        if (!isOurs && name !== "pre-commit.old") continue;
    ...
    if (touched) removed.push(entry.name);
  }
  ...
  if (JSON_MODE) {
    console.log(JSON.stringify({
      ...
      note: "pre-commit gate removed; use `consolidate` / `corpus verify` intentionally",
    }, null, 2));
    return;
  }
  console.log(`\n${hdr("Git Hooks (retired)")}\n`);
  console.log("  Cogentia no longer installs pre-commit commit gates.");
  console.log("  Use intentional checks: `cogentia consolidate` or `corpus verify [--strict]`.");
  if (removed.length) {
    console.log(`  Removed legacy hooks from: ${removed.join(", ")}`);
  } else {
    console.log("  No legacy Cogentia pre-commit hooks found.");
  }
  console.log();
}
